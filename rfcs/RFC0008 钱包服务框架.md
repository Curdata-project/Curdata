...
stateDiagram
	...
	[*] --> Start: 启动钱包
	Start --> StoreUninital: 空钱包
	StoreUninital --> StoreInitaled: 初始化成功
	StoreInitaled --> Unregistered: 不存在已签名证书
	Unregistered --> Ready: 完成注册
	Ready --> Close: 收到关闭指令
	Ready --> Destory: 请空钱包
	Close --> [*]
	Destory --> [*]
	
	Start --> StoreInitaled: 已初始化
	
	StoreInitaled --> Ready: 已注册
```
